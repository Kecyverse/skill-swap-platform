...
import { UserWithSkills } from "@/app/page"; // We'll keep this type import from the homepage
import RequestSwapDialog from "./RequestSwapDialog"; // <-- Import the new dialog component
          ...
          <div className="flex flex-wrap gap-1">
            {user.skillsOffered.length > 0 ? (
              user.skillsOffered.map((s) => (
                <Badge key={s.skill.id}>{s.skill.name}</Badge>
              ))
            ) : (
          ...
          <div className="flex flex-wrap gap-1">
            {user.skillsWanted.length > 0 ? (
              user.skillsWanted.map((s) => (
                <Badge variant="secondary" key={s.skill.id}>
                  {s.skill.name}
                </Badge>
              ))
            ) : (
